...
J.ui.dialog = function( opts, elem )
{
	...
	if( r.html )
	{
	    if( typeof r.html === 'string' )
	        innerDoc = '<div id="inbox" class="inbox" style="display:none">' + r.html + '</div>';
		else
		    innerDoc = '<div id="inbox" class="inbox" style="display:none"></div>';
	}
	else if( r.page )
	{
	...
	text = [
	    '<div id="', r.id, '" class="lhgdig" style="width:', r.width, 'px;height:', r.height, 'px;">',
			...
			'<tr>',
				'<td class="leftTop"></td>',
				'<td id="drag" class="top">',
				    '<div class="title"><span id="icon" class="icon"></span>', r.title, '</div>',
					'<div id="xbtn" class="xbtn"></div>',
				'</td>',
				'<td class="rightTop"></td>',
			'</tr>',
			'<tr>',
				'<td class="left" id="digLeft"></td>',
				'<td>',
					'<table border="0" cellspacing="0" cellpadding="0">',
					'<tr>',
					    '<td id="content" class="content">',
						    innerDoc, '<div id="throbber" class="throbber"><span id="load">loading...</span></div>',
						'</td>',
					'</tr>',
					r.btns ? '<tr><td id="btns" class="btns"><div id="bDiv" class="bDiv"></div></td></tr>' : '',
					'</table>',
				'</td>',
				'<td class="right"></td>',
			'</tr>',
			'<tr>',
				...
				'<td class="bottom"></td>',
				'<td id="drop" class="rightBottom"></td>',
			'</tr>',
			'</table>', iframe,
		'</div>'
	].join('');
	...
	this.ShowDialog = function()
	{
		...
		this.lhgDigxW = J('#digLeft',this.dlg)[0].offsetWidth * 2;
		...
		if( r.html )
		{
			...
			J('#inbox',this.dlg)[0].style.display = 'inline-block';
		}
	};
	...
	{
	    var tH = J('#drag',dig)[0].offsetHeight,
		    bH = J('#drop',dig)[0].offsetHeight,
			xW = this.lhgDigxW,
			nH = r.btns ? J('#btns',dig)[0].offsetHeight : 0,
	...
	{
	    var loadL = ( J('#content',this.dlg)[0].offsetWidth -
		    J('#load',this.dlg)[0].offsetWidth ) / 2;
			
		J('#load',this.dlg)[0].style.left = loadL + 'px';
	};
	...
	this.addBtn = function( id, txt, fn )
	{
		...
		{
		    var html = '<a id="' + id + '" class="button" href="javascript:void(0)" hidefocus="true"><em>' + txt + '</em></a>',
		
		    btn = J(html,doc).click(fn)[0];
		    J('#bDiv',this.dlg).append( btn );
		}
	};
